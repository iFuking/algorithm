...
public:
	int b_search(vector<pair<int, int>>& nodes, int left, int right, int n)
	{
		while (left <= right) {
			...
			if (nodes[mid].second < n) left = mid+1;
			else if (nodes[mid].second > n) right = mid-1;
			else return nodes[mid].first;
		}
		return -1;
	}
    ...
    vector<int> twoSum(vector<int>& nums, int target) {
		...
		for (int i = 0; i<nodes.size()-1 && nodes[i].second<=target/2; ++i) {
			int index2 = b_search(nodes, i+1, nodes.size()-1, target-nodes[i].second);
			if (index2 >= 0) {
				ans.push_back(min(nodes[i].first, index2)); ans.push_back(max(nodes[i].first, index2));
				break;
			}
		}
		return ans;
